...
router.post("/", authGovernmentOnly, function (req, res, next) {
    ...
    if (
        Object.entries(requiredFields).some(([field, type]) => typeof contribData[field] !== type)
    ) {
    ...
    const success = databaseConnector.addBusinessContribution(projectId, contribData);
    if (!success) {
        return res.status(500).json({ success: false, message: "Failed to add business contribution" });
    }
